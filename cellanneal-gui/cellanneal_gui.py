...
class cellgui:
    def __init__(self, root):
        ...
        self.logo_label.grid(
            column=1, columnspan=1, sticky=tk.W, row=0, padx=5, pady=5, rowspan=2
        )
        ...
        self.welcome_label.grid(
            column=2, columnspan=4, sticky=tk.W + tk.E, row=0, pady=10
        )

        # a button that allows to show a small documentation
        self.instruct_button = tk.Button(
            root,
            text="Instructions",
        ...
        self.param_label = tk.Label(
            root, text="deconvolution data", font="-weight bold"
        )
        ...
        self.bulk_browse_button = tk.Button(
            root, text="Browse file system", command=lambda: self.import_bulk_data()
        )
        self.bulk_browse_button.grid(
            row=i_i, column=4, columnspan=2, sticky=tk.W + tk.E
        )

        # import celltype data
        # label to indicate that we want celltype data here
        self.celltype_data_label = tk.Label(
            root, text="Select signature data (.csv, .txt, .xlsx)."
        )
        self.celltype_data_label.grid(row=i_i + 1, column=1, columnspan=2, sticky=tk.W)
        # path entry field
        self.celltype_data_entry = tk.Entry(
            root, textvariable=self.celltype_data_path, state="readonly"
        )
        self.celltype_data_entry.grid(
            row=i_i + 1, column=2, columnspan=2, sticky=tk.W + tk.E
        )
        # file system browse button
        self.celltype_browse_button = tk.Button(
            root, text="Browse file system", command=lambda: self.import_celltype_data()
        )
        self.celltype_browse_button.grid(
            row=i_i + 1, column=4, columnspan=2, sticky=tk.W + tk.E
        )

        # select output folder
        # label to indicate that we want celltype data here
        self.output_folder_label = tk.Label(
            root, text="Select folder to store results."
        )
        self.output_folder_label.grid(row=i_i + 2, column=1, columnspan=2, sticky=tk.W)
        # path entry field
        self.output_folder_entry = tk.Entry(
            root, textvariable=self.output_path, state="readonly"
        )
        self.output_folder_entry.grid(
            row=i_i + 2, column=2, columnspan=2, sticky=tk.W + tk.E
        )
        # file system browse button
        self.celltype_browse_button = tk.Button(
            root, text="Browse file system", command=lambda: self.select_output_folder()
        )
        self.celltype_browse_button.grid(
            row=i_i + 2, column=4, columnspan=2, sticky=tk.W + tk.E
        )

        """ parameter section """
        # title
        self.param_label = tk.Label(
            root, text="deconvolution parameters", font="-weight bold"
        )
        self.param_label.grid(row=p_i - 1, column=1, columnspan=3, sticky=tk.W)
        # list parameter settings with set of labels
        self.bulk_min_param_label = tk.Label(root, text="minimum expression in mixture")
        self.bulk_min_param_label.grid(row=p_i, column=1, sticky=tk.W)

        self.bulk_max_param_label = tk.Label(root, text="maximum expression in mixture")
        self.bulk_max_param_label.grid(row=p_i + 1, column=1, sticky=tk.W)

        self.disp_min_param_label = tk.Label(root, text="minimum scaled dispersion")
        self.disp_min_param_label.grid(row=p_i + 2, column=1, sticky=tk.W)

        self.maxiter_param_label = tk.Label(root, text="maximum number of iterations")
        self.maxiter_param_label.grid(row=p_i + 3, column=1, sticky=tk.W)

        # corresponding set of values
        self.bulk_min_value_label = tk.Label(root, text="{}".format(self.bulk_min))
        self.bulk_min_value_label.grid(row=p_i, column=2, sticky=tk.W)

        self.bulk_max_value_label = tk.Label(root, text="{}".format(self.bulk_max))
        self.bulk_max_value_label.grid(row=p_i + 1, column=2, sticky=tk.W)

        self.disp_min_value_label = tk.Label(root, text="{}".format(self.disp_min))
        self.disp_min_value_label.grid(row=p_i + 2, column=2, sticky=tk.W)

        self.maxiter_value_label = tk.Label(root, text="{}".format(self.maxiter))
        self.maxiter_value_label.grid(row=p_i + 3, column=2, sticky=tk.W)

        # button for editing parameters which spawns new window
        self.parameter_change_button = tk.Button(
            root,
            text="Change\nparameters",
        ...
        self.parameter_change_button.grid(
            row=p_i,
            rowspan=2,
            column=3,
            columnspan=2,
        ...
        self.cellanneal_button.grid(
            row=d_i, column=1, columnspan=5, sticky=tk.W + tk.E, padx=10, pady=50
        )
    ...
    def show_instructions(self):
        ...
        wl = 700
        self.readme_text0 = tk.Label(
            readme_window, wraplength=wl, justify=tk.LEFT, text=""" """
        )
        self.readme_text0.grid(row=13, column=0, padx=10)

        self.readme_text1 = tk.Label(
            readme_window,
            wraplength=wl,
            text="Welcome to cellanneal.",
            font="-weight bold",
        )
        self.readme_text1.grid(row=1, column=0, padx=10)

        self.readme_text2 = tk.Label(
            readme_window,
            wraplength=wl,
            text="For the full documentation and example files please see https://github.com/LiBuchauer/cellanneal.",
        )
        self.readme_text2.grid(row=2, column=0, padx=10)

        self.readme_text3 = tk.Label(
            readme_window,
            wraplength=wl,
        ...
        self.readme_text12 = tk.Label(
            readme_window,
            wraplength=wl,
            justify=tk.LEFT,
            text="""Click on "run cellanneal" and watch the progress in the accompanying console. The main window freezes while deconvolution is in progress. Result tables and figures are stored to the user-specified output folder.""",
        )
        self.readme_text12.grid(row=12, column=0, padx=10)

        self.readme_text13 = tk.Label(
            readme_window, wraplength=wl, justify=tk.LEFT, text=""" """
        )
    ...
    def open_param_window(self):
        ...
        # entry field
        self.bulk_min_entry = tk.Entry(par_window, width=8)
        ...
        self.bulk_min_set_button = tk.Button(
            par_window, text="set", command=lambda: self.set_bulk_min(), width=8
        )
        self.bulk_min_set_button.grid(row=3, column=2, sticky=tk.W)

        # for parameter bulk_max
        # title label and current value
        self.bulk_max_label = tk.Label(
            par_window, text="maximum expression in mixture", font="-weight bold"
        )
        self.bulk_max_label.grid(row=4, column=1, columnspan=2, sticky=tk.W + tk.E)

        # entry field
        self.bulk_max_entry = tk.Entry(par_window, width=8)
        ...
        self.bulk_max_set_button = tk.Button(
            par_window, text="set", command=lambda: self.set_bulk_max(), width=8
        )
        self.bulk_max_set_button.grid(row=6, column=2, sticky=tk.W)

        # for parameter disp_min
        # title label and current value
        self.disp_min_label = tk.Label(
            par_window, text="minimum scaled dispersion", font="-weight bold"
        )
        self.disp_min_label.grid(row=7, column=1, columnspan=2, sticky=tk.W + tk.E)

        # entry field
        self.disp_min_entry = tk.Entry(par_window, width=8)
        ...
        self.disp_min_set_button = tk.Button(
            par_window, text="set", command=lambda: self.set_disp_min(), width=8
        )
        self.disp_min_set_button.grid(row=9, column=2, sticky=tk.W)

        # for parameter maxiter
        # title label and current value
        self.maxiter_label = tk.Label(
            par_window, text="maximum number of iterations", font="-weight bold"
        )
        self.maxiter_label.grid(row=10, column=1, columnspan=2, sticky=tk.W + tk.E)

        # entry field
        self.maxiter_entry = tk.Entry(par_window, width=8)
        ...
        self.maxiter_set_button = tk.Button(
            par_window, text="set", command=lambda: self.set_maxiter(), width=8
        )
    ...
    def set_bulk_min(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Input error",
                """Please provdide a numerical value between 0 (inclusive) and 1 (exclusive). Examples: "0", "0.01", "2e-5".""",
            )
            return 0
        ...
        if (0 <= new_val < 1) and (new_val < self.bulk_max):
            ...
            messagebox.showerror(
                "Input error", """Your value must be smaller than bulk_max."""
            )
    ...
    def set_bulk_max(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Input error",
                """Please provdide a numerical value between 0 (exclusive) and 1 (inclusive). Examples: "0.01", "2e-5", "1".""",
            )
            return 0
        ...
        if (0 < new_val <= 1) and (new_val > self.bulk_min):
            ...
            messagebox.showerror(
                "Input error", """Your value must be larger than bulk_min."""
            )
    ...
    def set_disp_min(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Input error",
                """Please provdide a positive value. Examples: "0", "0.5", "2.7".""",
            )
            return 0
    ...
    def set_maxiter(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Input error",
                """Please provdide a positive integer. Examples: "50", "587", "1000".""",
            )
            return 0
        # check if value is >0
        if new_val >= 1:
    ...
    def cellanneal(self):
        ...
        if self.bulk_df_is_set == 0:
            messagebox.showerror(
                "Data error", """Please select a mixture data file in section 1)."""
            )
            return 0
        if self.celltype_df_is_set == 0:
            messagebox.showerror(
                "Data error", """Please select a signature data file in section 1)."""
            )
            return 0
        if self.output_path_is_set == 0:
            messagebox.showerror(
                "Data error",
                """Please select a folder for storing results in section 1).""",
            )
            return 0
        ...
        try:
            ...
            if bfile.split(".")[-1] in ["csv", "txt"]:
                self.bulk_df = read_csv(bfile, index_col=0, sep=None)
            ...
            messagebox.showerror(
                "Import error",
                """Your mixture data file could not be imported. Please check the documentation for format requirements and look at the example mixture data files.""",
            )
            print("+++ Aborted. +++")
            return 0
        ...
        try:
            ...
            if cfile.split(".")[-1] in ["csv", "txt"]:
                self.celltype_df = read_csv(cfile, index_col=0, sep=None)
            ...
            messagebox.showerror(
                "Import error",
                """Your signature data file could not be imported. Please check the documentation for format requirements and look at the example signature data files.""",
            )
            print("+++ Aborted. +++")
            return 0
